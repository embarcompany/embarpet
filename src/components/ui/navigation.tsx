...
export function SiteHeader({ logoSrc, items, cta, utilityItems = [], activeLabel, overlay = false, showLanguage = true, showMobileJourney = true, mobileCtaLabel, onCtaClick }: { logoSrc: string; items?: NavigationItem[]; cta?: NavigationLink; utilityItems?: NavigationLink[]; activeLabel?: string; overlay?: boolean; showLanguage?: boolean; showMobileJourney?: boolean; mobileCtaLabel?: string; onCtaClick?: () => void }) {
  ...
  const standardItems = useMemo<NavigationItem[]>(() => [
    ...
    { label:text.navDestinations, href:path("/#destinos"), children:[
      ...
      { label:"Portugal", href:path("/destinos/portugal"), description:"Planejamento para entrada na Europa.", flagSrc:countryFlagSvg("PT") },
      { label:"Espanha", href:path("/destinos/espanha"), description:"Requisitos definidos pela rota e pelo pet.", flagSrc:countryFlagSvg("ES") },
      { label:"Itália", href:path("/destinos/italia"), description:"Uma leitura própria antes do embarque.", flagSrc:countryFlagSvg("IT") },
      { label:"Argentina", href:path("/destinos/argentina"), description:"Planejamento regional com atenção ao caso.", flagSrc:countryFlagSvg("AR") },
      { label:"Uruguai", href:path("/destinos/uruguai"), description:"Documentação e operação conectadas.", flagSrc:countryFlagSvg("UY") },
      { label:"Paraguai", href:path("/destinos/paraguai"), description:"A rota orienta as próximas etapas.", flagSrc:countryFlagSvg("PY") },
  ...
  return <>
    <header className={cn("ep-site-header", overlay && "ep-site-header--overlay", !showMobileJourney && "ep-site-header--compact-mobile")}><nav className="ep-site-nav ep-container" aria-label="Main navigation">
      ...
      <div className="ep-site-nav__right"><div className="ep-nav-utilities">{showLanguage ? <LanguageSelector /> : null}{utilityItems.map((item) => { const UtilityIcon = item.icon; return <a key={item.label} href={item.href}>{UtilityIcon ? <UtilityIcon size={15} /> : null}{item.label}</a>; })}</div><a className="ep-nav-cta" href={resolvedCta.href} onClick={(event) => { if (onCtaClick || (resolvedCta.href === path("/analise") && isHome)) { event.preventDefault(); onCtaClick ? openPrimaryCta() : openMobilePlanner(); } }}><span>{resolvedCta.label}</span><img className="ep-nav-cta__plane" src="/embarpet-cta-plane-top.webp" alt="" aria-hidden="true" /></a></div>
