...
/**
 * IntegraÃ§Ã£o do prompt RideBookingForm.
 * MantÃ©m sua arquitetura: rota conectada, contexto local, data e CTA.
 * A adaptaÃ§Ã£o troca preÃ§o imediato por anÃ¡lise individual de viagem do pet.
 */
export const RideBookingForm = React.forwardRef<HTMLDivElement, RideBookingFormProps>(
  ({ className, imageUrl, city = "SÃ£o Paulo, Brasil", onSearch, ...props }, ref) => {
    ...
    return <div ref={ref} className={cn("w-full max-w-6xl mx-auto p-4 lg:p-7", className)} {...props}>
      <div className="grid grid-cols-1 lg:grid-cols-2 items-stretch overflow-hidden rounded-ember border border-embarpets-deep bg-card">
        <div className="p-5 sm:p-8 lg:p-10">
          <p className="mb-6 flex items-center gap-2 text-xs font-semibold text-embarpets-muted"><LocateFixed className="h-4 w-4 text-embarpets-deep" />VocÃª parte de {city}? <button type="button" className="font-bold text-embarpets-teal underline underline-offset-4">Alterar cidade</button></p>
          <div className="mb-7"><p className="text-xs font-extrabold uppercase tracking-[.15em] text-embarpets-deep">DiagnÃ³stico de viagem</p><h2 className="mt-2 text-4xl font-bold tracking-tight text-embarpets-teal sm:text-5xl">Vamos entender a rota do seu pet.</h2></div>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="relative border border-border bg-muted/50 p-3 sm:p-4">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><label className="flex items-center gap-3 border border-border bg-muted/50 px-4 py-3 text-sm text-foreground"><Calendar className="h-5 w-5 text-embarpets-deep" /><select aria-label="Quando pretende viajar?" value={date} onChange={(event) => setDate(event.target.value)} className="w-full bg-transparent outline-none"><option value="">Quando pretende viajar?</option><option>Nos prÃ³ximos 3 meses</option><option>Entre 3 e 6 meses</option><option>Mais de 6 meses</option><option>Ainda estou planejando</option></select></label><div className="flex items-center gap-3 border border-border bg-muted/50 px-4 py-3 text-xs font-semibold text-embarpets-muted"><ShieldCheck className="h-5 w-5 shrink-0 text-embarpets-deep" />AnÃ¡lise personalizada</div></div>
            <div className="flex flex-wrap items-center gap-4 pt-2"><button type="submit" className="inline-flex h-12 items-center justify-center gap-2 rounded-ember bg-primary px-7 text-sm font-extrabold text-primary-foreground hover:bg-primary/90">Continuar anÃ¡lise <span aria-hidden="true">â†’</span></button><span className="text-xs leading-relaxed text-embarpets-muted">A modalidade depende da rota, do pet e das regras vigentes.</span></div>
          </form>
        </div>
        <div className="hidden min-h-[540px] p-6 lg:block"><img src={imageUrl} alt="Tutor com pet em contexto de viagem" className="h-full w-full rounded-ember border border-border object-cover" /></div>
      </div>
    </div>;
  },
);
