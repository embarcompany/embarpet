...
import { ArrowRight, ClipboardCheck, Crown, FileText, Globe2, Headset, HeartHandshake, MapPin, Package, Pause, Plane, Play, Route, ShieldCheck, Users, Volume2, VolumeX, X } from "lucide-react";
...
export default function EmbarpetHome() {
  ...
  return <>{analysisOpen ? <div className="ep-analysis-modal" role="dialog" aria-modal="true" aria-label={text.startAnalysis}><div className="ep-analysis-modal__backdrop" onClick={closeAnalysis} /><section className="ep-analysis-modal__panel"><header><a href={path("/")} aria-label="Embarpet"><img src="/logo-embarpet-dark.png" alt="Embarpet" /></a><button type="button" onClick={closeAnalysis} aria-label={text.close}><X size={21} /></button></header><div className="ep-analysis-modal__content"><DiagnosticFlow routeFirst startAtPet={Boolean(analysisRoute.origin && analysisRoute.destination && analysisRoute.period)} initialRoute={analysisRoute} analyticsSource="index_modal" /></div></section></div> : null}<SiteHeader overlay logoSrc="/logo-embarpet-dark.png" items={[
    ...
    <ConversionHero aside={<aside className="ep-hero-showcase" aria-label="Embarpet em operação"><div className="ep-hero-showcase__gradient" aria-hidden="true" /><img className="ep-hero-showcase__pet" src="/embarpet-hero-dachshund-viagem-trim.png" alt="Cachorro com mala e passaporte brasileiro para uma viagem internacional" /><section className="ep-hero-showcase__vsl"><div className="ep-hero-showcase__video"><video ref={heroVideoRef} key={heroVideoFullLoaded ? "hero-full" : "hero-preview"} poster="/embarpet-hero-vsl-poster.jpg" aria-label="Acompanhamento Embarpet em contexto de viagem" autoPlay loop muted={heroVideoFullLoaded ? !heroVideoUnmuted : true} playsInline preload="metadata" onPause={() => setHeroVideoPaused(true)} onPlay={() => setHeroVideoPaused(false)}>{!heroVideoFullLoaded ? <source src="/embarpet-hero-vsl-preview-5s.webm" type="video/webm" /> : null}<source src="/embarpet-hero-vsl.mp4" type="video/mp4" /></video><button type="button" aria-label={heroVideoUnmuted ? (heroVideoPaused ? "Reproduzir vídeo" : "Pausar vídeo") : "Ativar som"} className={`ep-hero-video__sound${heroVideoUnmuted ? " is-playing" : ""}`} onClick={() => { if (!heroVideoFullLoaded) { setHeroVideoFullLoaded(true); setHeroVideoUnmuted(true); setHeroVideoPaused(false); return; } if (!heroVideoUnmuted) { setHeroVideoUnmuted(true); setHeroVideoPaused(false); void heroVideoRef.current?.play(); return; } if (heroVideoRef.current?.paused) void heroVideoRef.current.play(); else heroVideoRef.current?.pause(); }}>{heroVideoUnmuted ? (heroVideoPaused ? <Play size={14} /> : <Pause size={14} />) : <><Volume2 size={15} />Ativar som</>}</button></div></section></aside>}>
    ...
    <section className="ep-home-modalities" id="modalidades"><div className="ep-container"><div className="ep-home-modalities__intro"><div><p className="ep-eyebrow">Antes de escolher</p><h2 className="ep-title-lg">Antes da modalidade, vem a leitura do seu caso.</h2></div><p className="ep-copy">Destino, perfil do pet, companhia aérea e documentação definem qual alternativa pode fazer sentido para a viagem.</p></div>
      <ModalityRail items={[
        { icon:"plane", title:"Viagem na cabine", copy:"Em algumas rotas, pets dentro dos critérios podem viajar próximos ao tutor. A possibilidade depende do pet, da companhia aérea e das regras vigentes.", ctaLabel:"Avaliar possibilidade de cabine", detailHref:"#analise", imageSrc:"/embarpet-viagem-cabine.jpg", imageAlt:"Pet olhando pela janela durante um voo em cabine" },
        { icon:"document", title:"Bagagem acompanhada", copy:"Em determinados casos, o pet embarca no mesmo voo do tutor, em compartimento apropriado. Rota, caixa e critérios operacionais orientam a análise.", ctaLabel:"Avaliar bagagem acompanhada", detailHref:"#analise", imageSrc:"/embarpet-bagagem-acompanhada.webp", imageAlt:"Consultora Embarpet acompanhando pets em aeroporto" },
        { icon:"home", title:"Compartimento de cargas", copy:"Uma operação especializada para perfis, rotas ou viagens que pedem uma estrutura própria. A viabilidade é avaliada conforme a jornada e os requisitos vigentes.", ctaLabel:"Avaliar compartimento de cargas", detailHref:"#analise", imageSrc:"/embarpet-compartimento-cargas.jpg", imageAlt:"Pet em caixa de transporte na esteira operacional do aeroporto", featured:true },
        { icon:"shield", title:"Suporte emocional", copy:"Em situações aplicáveis, avaliamos o contexto da viagem e os critérios da companhia aérea para orientar os próximos passos. Não é uma garantia de embarque.", ctaLabel:"Avaliar suporte emocional", detailHref:"#analise", imageSrc:"/embarpet-suporte-emocional.jpg", imageAlt:"Família com pet em voo durante uma viagem" },
      ]} />
      <p className="ep-home-bridge"><ShieldCheck size={18} aria-hidden="true" /><span>A melhor possibilidade nasce da análise certa — e é isso que começamos a fazer com você.</span></p>
    </div></section>

    <section className="ep-home-service" id="servicos"><div className="ep-container ep-home-service__grid"><figure className="ep-home-ceo" aria-label="Thamires Félix, cofundadora da Embarpet"><div className="ep-home-ceo__glow" aria-hidden="true" /><img className="ep-home-ceo__portrait" src="/embarpet-thamires-felix.webp" alt="Thamires Félix, cofundadora da Embarpet" /><span className="ep-home-ceo__fade" aria-hidden="true" /><figcaption>Thamires Félix <small>Embarpet</small></figcaption></figure><div className="ep-home-service__content"><p className="ep-eyebrow">Quem cuida da viagem</p><p className="ep-home-service__identity"><strong>Thamires Félix</strong><span>Cofundadora da Embarpet</span></p><h2 className="ep-title-lg">Sua viagem internacional merece <strong>clareza antes de cada decisão.</strong></h2><p className="ep-copy">À frente da Embarpet, Thamires conecta <strong>atendimento, documentação e operação</strong> para transformar uma jornada complexa em próximos passos claros. Hoje, lidera uma equipe de <strong>mais de 30 especialistas</strong> dedicada a planejar cada embarque com o cuidado que o seu pet merece.</p><ul className="ep-home-service__proofs"><li><ShieldCheck size={17} /><span><strong>Leitura técnica</strong><small>Rota, prazo e exigências</small></span></li><li><Users size={17} /><span><strong>+30 especialistas</strong><small>Equipe conectada à sua rota</small></span></li><li><Headset size={17} /><span><strong>Acompanhamento próximo</strong><small>Do início ao embarque</small></span></li></ul><a className="ep-button ep-button--secondary ep-internal-cta ep-internal-cta--on-dark" href="/equipe-e-credenciais"><span>Conhecer a equipe Embarpet</span><i className="ep-internal-cta__arrow" aria-hidden="true"><ArrowRight size={16} /></i></a></div></div></section>

    <section className="ep-home-process-section" id="como-funciona"><div className="ep-container ep-home-process ep-home-process--cycle"><div className="ep-home-process__intro"><p className="ep-eyebrow">Nosso processo</p><h2 className="ep-title-lg">Transformamos complexidade em <em>caminho.</em></h2><p className="ep-copy">Conectamos rota, perfil do pet, exigências e operação para que cada decisão tenha contexto antes de virar próximo passo.</p><a className="ep-home-process__cta" href="#analise"><span>Começar minha análise</span><ArrowRight size={18} aria-hidden="true" /></a></div><div className="ep-home-process__cycle" aria-label="As quatro etapas da metodologia Embarpet"><span className="ep-home-process__orbit" aria-hidden="true" /><div className="ep-home-process__core"><img src="/logo-embarpet-symbol.png" alt="" /><strong>Método<br />Embarpet</strong><small>Uma jornada<br />com contexto.</small></div>{[
