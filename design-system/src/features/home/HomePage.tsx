...
export default function EmbarpetHome() {
  ...
  return <>{analysisOpen ? <div className="ep-analysis-modal" role="dialog" aria-modal="true" aria-label={text.startAnalysis}><div className="ep-analysis-modal__backdrop" onClick={closeAnalysis} /><section className="ep-analysis-modal__panel"><header><a href={path("/")} aria-label="Embarpet"><img src="/logo-embarpet-dark.png" alt="Embarpet" /></a><button type="button" onClick={closeAnalysis} aria-label={text.close}><X size={21} /></button></header><div className="ep-analysis-modal__content"><DiagnosticFlow routeFirst startAtPet={Boolean(analysisRoute.origin && analysisRoute.destination && analysisRoute.period)} initialRoute={analysisRoute} analyticsSource="index_modal" /></div></section></div> : null}<SiteHeader overlay logoSrc="/logo-embarpet-dark.png" items={[
    ...
    <ConversionHero aside={<aside className="ep-hero-showcase" aria-label="Embarpet em operação"><div className="ep-hero-showcase__gradient" aria-hidden="true" /><img className="ep-hero-showcase__thamires" src="/embarpet-ceo-equipe-trim.webp" alt="Thamires Félix com pets atendidos pela Embarpet" /><section className="ep-hero-showcase__vsl"><span>Veja a Embarpet em operação</span><div className="ep-hero-showcase__video"><video key={heroVideoFullLoaded ? "hero-full" : "hero-preview"} poster="/embarpet-pet-luxo-real.jpeg" aria-label="Acompanhamento Embarpet em contexto de viagem" autoPlay loop muted={heroVideoFullLoaded ? !heroVideoUnmuted : true} playsInline preload="metadata">{!heroVideoFullLoaded ? <source src="/embarpet-petluxo-preview-5s.webm" type="video/webm" /> : null}<source src={heroVideoFullLoaded ? "/embarpet-petluxo-baeta-alpargata.mp4" : "/embarpet-petluxo-preview-5s.mp4"} type="video/mp4" /></video><button type="button" className="ep-hero-video__sound" onClick={() => { if (!heroVideoFullLoaded) { setHeroVideoFullLoaded(true); setHeroVideoUnmuted(true); return; } setHeroVideoUnmuted((current) => !current); }}>{heroVideoUnmuted ? <VolumeX size={15} /> : <Volume2 size={15} />}{heroVideoUnmuted ? "Silenciar" : "Ativar som"}</button></div></section></aside>}>
