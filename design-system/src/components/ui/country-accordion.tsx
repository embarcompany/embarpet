...
export function InternationalTransfer({ subtitle, title, description, regions, onCountrySelect, onOtherDestination }: InternationalTransferProps) {
  ...
  return <section className="ep-home-destinations" id="destinos" aria-label="Destinos internacionais"><div className="ep-destination-selector">
    ...
    <div className="ep-destination-selector__panel"><div className="ep-destination-selector__panel-title"><span><Globe2 size={22} /></span><div><b>Escolha seu destino</b><small>Selecione uma rota para começarmos.</small></div></div><div className="ep-destination-selector__choices">{countries.map((country) => <button type="button" key={country.code} onClick={() => onCountrySelect?.(country)}><img src={countryFlagSvg(country.code)} alt="" width="40" height="30" loading="lazy" /><span>{country.name}</span><ArrowRight size={17} /></button>)}<button type="button" className="ep-destination-selector__other" onClick={() => { if (onOtherDestination) onOtherDestination(); else document.getElementById("analise")?.scrollIntoView({ behavior:"smooth" }); }}><i><Globe2 size={20} /></i><span>Outros destinos</span><ArrowRight size={17} /></button></div></div>
  </div></section>;
}
