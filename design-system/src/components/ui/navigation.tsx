...
export function SiteHeader({ logoSrc, items, cta, utilityItems = [], activeLabel, overlay = false }: { logoSrc: string; items: NavigationItem[]; cta?: NavigationLink; utilityItems?: NavigationLink[]; activeLabel?: string; overlay?: boolean }) {
  ...
  return <>
    ...
    <header className={cn("ep-site-header", overlay && "ep-site-header--overlay")}><nav className="ep-site-nav ep-container" aria-label="Main navigation">
      ...
      {desktopRouteVisible ? <div className="ep-desktop-route-dock" aria-label="Inicie sua análise de viagem"><div className="ep-container"><label><MapPin size={17} aria-hidden="true" /><span>De onde seu pet parte?</span><input value={desktopRoute.origin} onChange={(event) => setDesktopRoute((current) => ({ ...current, origin:event.target.value }))} placeholder="Digite uma cidade" /></label><Route className="ep-desktop-route-dock__connector" size={18} aria-hidden="true" /><label><MapPin size={17} aria-hidden="true" /><span>Para onde seu pet vai?</span><input value={desktopRoute.destination} onChange={(event) => setDesktopRoute((current) => ({ ...current, destination:event.target.value }))} placeholder="Digite uma cidade" /></label><button type="button" onClick={() => openMobilePlanner(desktopRoute)}>Iniciar análise <ArrowRight size={17} aria-hidden="true" /></button></div></div> : null}
