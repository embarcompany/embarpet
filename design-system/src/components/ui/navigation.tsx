...
export function SiteHeader({ logoSrc, items, cta, utilityItems = [], activeLabel, overlay = false }: { logoSrc: string; items: NavigationItem[]; cta?: NavigationLink; utilityItems?: NavigationLink[]; activeLabel?: string; overlay?: boolean }) {
  ...
  return <>
    ...
    {mobilePlannerOpen ? <div className="ep-mobile-planner" role="dialog" aria-modal="true" aria-label={text.startAnalysis}><div className="ep-mobile-planner__panel"><button type="button" className="ep-mobile-planner__close" onClick={() => setMobilePlannerOpen(false)} aria-label={text.close}><X size={19} /></button><DiagnosticFlow routeFirst /></div></div> : null}
  </>;
}
