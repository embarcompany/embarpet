...
export function SiteHeader({ logoSrc, items, cta = { label:"Analisar a viagem", href:"#analise" }, utilityItems = [{ label:"PT-BR", href:"#", icon:Globe2 },{ label:"Dúvidas", href:"#faq" }], activeLabel, overlay = false }: { logoSrc: string; items: NavigationItem[]; cta?: NavigationLink; utilityItems?: NavigationLink[]; activeLabel?: string; overlay?: boolean }) {
  ...
  return <>{noticeOpen ? <div className="ep-site-notice" role="note"><AlertTriangle size={14} /><p><b>Aviso importante:</b> atendemos somente destinos internacionais. Voos nacionais são auxiliados apenas quando conectam a uma viagem internacional.</p><button type="button" onClick={dismissNotice} aria-label="Fechar aviso"><X size={15} /></button></div> : null}<header className={cn("ep-site-header", overlay && "ep-site-header--overlay")}><nav className="ep-site-nav ep-container" aria-label="Navegação principal">
    ...
    <button className="ep-nav-mobile-cta" type="button" onClick={openMobilePlanner}><span>Começar viagem</span></button>
  ...
  </nav><div className="ep-mobile-journey"><div className="ep-mobile-shortcuts" aria-label="Atalhos de navegação"><a href="#como-funciona"><ClipboardCheck size={16} /><span>Como funciona</span></a><a href="#modalidades"><Route size={16} /><span>Modalidades</span></a><a href="#destinos"><MapPin size={16} /><span>Destinos</span></a><a href="#historias"><HeartHandshake size={16} /><span>Histórias</span></a></div><div className="ep-mobile-route-search"><MapPin size={17} aria-hidden="true" /><button className="ep-mobile-route-search__trigger" type="button" onClick={openMobilePlanner}>Para onde seu pet vai?</button><button className="ep-mobile-route-search__cta" type="button" onClick={openMobilePlanner}>Começar viagem</button></div></div>{mobileOpen ? <div className="ep-mobile-menu"><div className="ep-container">{items.map((item) => <div key={item.label}><a href={item.href} onClick={() => setMobileOpen(false)}>{item.label}</a>{item.children?.map((child) => <a key={child.label} className="ep-mobile-menu__child" href={child.href} onClick={() => setMobileOpen(false)}>{child.label}</a>)}</div>)}<button className="ep-nav-cta" type="button" onClick={openMobilePlanner}>{cta.label} →</button></div></div> : null}</header>{mobilePlannerOpen ? <div className="ep-mobile-planner" role="dialog" aria-modal="true" aria-label="Iniciar análise da viagem"><div className="ep-mobile-planner__panel"><button type="button" className="ep-mobile-planner__close" onClick={() => setMobilePlannerOpen(false)} aria-label="Fechar análise"><X size={19} /></button><DiagnosticFlow routeFirst /></div></div> : null}</>;
}
