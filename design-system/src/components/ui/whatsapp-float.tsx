...
export function WhatsAppFloat({ context, onStart }: { context: LeadContext; onStart?: (context: LeadContext) => void }) {
  ...
  const start = () => { onStart?.(context); setOpen(false); setShowNudge(false); };
  return <aside className="ep-whatsapp-float" aria-label="Ajuda pelo WhatsApp">{showNudge && !open ? <button type="button" className="ep-whatsapp-nudge" onClick={() => { setOpen(true); setShowNudge(false); }}><b>A Jadi pode ajudar</b><span>com o embarque do seu pet</span></button> : null}{open ? <div className="ep-whatsapp-popover"><button className="ep-whatsapp-close" type="button" aria-label="Fechar conversa" onClick={() => setOpen(false)}><X size={16} /></button><span className="ep-whatsapp-avatar"><img src="https://cdn.simpleicons.org/whatsapp/25D366" alt="WhatsApp" /></span><p><b>Precisa de ajuda com a viagem?</b><small>A Jadi e a equipe continuam com o contexto da sua rota.</small></p><button type="button" onClick={start}>Conversar com a Embarpet <span aria-hidden="true">→</span></button></div> : null}<button className="ep-whatsapp-trigger" type="button" aria-label="Abrir ajuda pelo WhatsApp" aria-expanded={open} onClick={() => { setOpen((value) => !value); setShowNudge(false); }}><img src="https://cdn.simpleicons.org/whatsapp/ffffff" alt="" /></button></aside>;
}
