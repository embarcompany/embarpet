...
export type CaseCard = { id: string; title: string; subtitle: string; imageSrc: string; imageAlt: string; videoSrc?: string; previewWebmSrc?: string; fullVideoSrc?: string; instagramHandle?: string; instagramUrl?: string };
...
function CaseVideo({ item, unmuted, loop = false }: { item: CaseCard; unmuted: boolean; loop?: boolean }) {
  const useFullVideo = unmuted && Boolean(item.fullVideoSrc);
  const mp4Src = useFullVideo ? item.fullVideoSrc : item.videoSrc;
  return <video className="ep-case-card__image" poster={item.imageSrc} aria-label={item.imageAlt} autoPlay loop={loop} muted={!unmuted} playsInline preload="metadata" onTimeUpdate={(event) => { if (!useFullVideo && event.currentTarget.currentTime >= 5) event.currentTarget.currentTime = 0; }}>
    {!useFullVideo && item.previewWebmSrc ? <source src={item.previewWebmSrc} type="video/webm" /> : null}
...
export function CaseDragCards({ cases }: { cases: CaseCard[] }) {
  ...
  if (isMobile) return <div className="ep-case-carousel" aria-label="Histórias de famílias atendidas pela Embarpet">
    <div className="ep-case-carousel__heading"><div className="ep-case-canvas__proof"><p className="ep-eyebrow"><strong>+2.000</strong> embarques acompanhados</p><span className="ep-case-google"><img src="/logo-google.svg" alt="Google" /><b>4,9</b><span aria-label="4,9 estrelas no Google"><Star size={9} fill="currentColor" /><Star size={9} fill="currentColor" /><Star size={9} fill="currentColor" /><Star size={9} fill="currentColor" /><Star size={9} fill="currentColor" /></span></span></div><h2>Histórias de quem<br />confiou na jornada.</h2><p>Relatos reais de jornadas acompanhadas pela Embarpet.</p></div>
    <div className="ep-case-carousel__viewport"><div className="ep-case-carousel__track" style={{ transform:`translateX(-${mobileIndex * 100}%)` }}>{mobileCases.map((item, index) => <article className={mobileIndex === index ? "ep-case-carousel__card is-active" : "ep-case-carousel__card"} key={item.id}>{item.videoSrc && mobileIndex === index ? <CaseVideo item={item} unmuted={Boolean(unmutedVideos[item.id])} loop /> : <img className="ep-case-card__image" src={item.imageSrc} alt={item.imageAlt} loading="lazy" decoding="async" />}<div className="ep-case-carousel__story-progress" aria-hidden="true">{mobileCases.map((story, storyIndex) => <i className={storyIndex < mobileIndex ? "is-complete" : storyIndex === mobileIndex ? "is-current" : ""} key={story.id} />)}</div>{item.videoSrc ? <button type="button" className="ep-case-card__sound" onClick={() => toggleVideoSound(item)}>{unmutedVideos[item.id] ? <VolumeX size={12} /> : <Volume2 size={12} />}{unmutedVideos[item.id] ? "Silenciar" : "Ativar som"}</button> : null}<span><b>{item.title}</b><small>{item.subtitle}</small>{item.instagramHandle && item.instagramUrl ? <a className="ep-case-card__instagram" href={item.instagramUrl} target="_blank" rel="noreferrer"><img src="https://upload.wikimedia.org/wikipedia/commons/a/a5/Instagram_icon.png" alt="" loading="lazy" decoding="async" />{item.instagramHandle}</a> : null}</span></article>)}</div></div>
    <a className="ep-case-carousel__more ep-cta ep-cta--navigate" href="/historias"><span>Veja mais histórias</span><i aria-hidden="true"><ArrowRight size={15} /></i></a>
  </div>;
  return <div className="ep-case-canvas" ref={areaRef} aria-label="Depoimentos de famílias atendidas. Arraste os cartões para explorar.">
    <div className="ep-case-canvas__center"><div className="ep-case-canvas__proof"><p className="ep-eyebrow"><strong>+2.000</strong> embarques acompanhados</p><span className="ep-case-google"><img src="/logo-google.svg" alt="Google" /><b>4,9</b><span aria-label="4,9 estrelas no Google"><Star size={9} fill="currentColor" /><Star size={9} fill="currentColor" /><Star size={9} fill="currentColor" /><Star size={9} fill="currentColor" /><Star size={9} fill="currentColor" /></span></span></div><h2>Histórias de quem<br />confiou na jornada.</h2><p>Arraste os cartões para explorar relatos.</p><a className="ep-cta ep-cta--navigate" href="/historias"><span>Veja mais histórias</span><i aria-hidden="true"><ArrowRight size={15} /></i></a></div>
    {cases.map((item, index) => { const position = currentPositions[item.id] ?? positions[index]; return <article ref={(node) => { cardRefs.current[item.id] = node; }} data-case-id={item.id} className={active === item.id ? "ep-case-card is-active" : "ep-case-card"} key={item.id} style={{ left: `${position.x}%`, top: `${position.y}%`, "--ep-case-rotation": `${rotations[index]}deg`, zIndex: topCard === item.id ? 20 : index + 1 } as CSSProperties} onPointerDown={(event) => startDrag(event, item)} onPointerMove={(event) => active === item.id && moveCard(event, item)} onPointerUp={() => setActive(null)} onPointerCancel={() => setActive(null)}>
      {item.videoSrc && loadedVideos[item.id] ? <CaseVideo item={item} unmuted={Boolean(unmutedVideos[item.id])} loop /> : <img className="ep-case-card__image" src={item.imageSrc} alt={item.imageAlt} draggable={false} loading="lazy" decoding="async" />}{item.videoSrc ? <button type="button" className="ep-case-card__sound" onPointerDown={(event) => event.stopPropagation()} onClick={() => toggleVideoSound(item)}>{unmutedVideos[item.id] ? <VolumeX size={12} /> : <Volume2 size={12} />}{unmutedVideos[item.id] ? "Silenciar" : "Ativar som"}</button> : null}<span><b>{item.title}</b><small>{item.subtitle}</small>{item.instagramHandle && item.instagramUrl ? <a className="ep-case-card__instagram" href={item.instagramUrl} target="_blank" rel="noreferrer" onPointerDown={(event) => event.stopPropagation()}><img src="https://upload.wikimedia.org/wikipedia/commons/a/a5/Instagram_icon.png" alt="" loading="lazy" decoding="async" />{item.instagramHandle}</a> : null}</span>
    </article>; })}
  </div>;
}
