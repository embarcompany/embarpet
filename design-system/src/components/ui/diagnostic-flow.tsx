...
export function DiagnosticFlow({ onComplete, onRouteChange, initialRoute, routeFirst = false, startAtPet = false, integrated = false, analyticsSource = routeFirst ? "mobile_header" : "hero_form" }: { onComplete?: (lead: PublicLead) => void; onRouteChange?: (route: RouteData) => void; initialRoute?: Partial<RouteData>; routeFirst?: boolean; startAtPet?: boolean; integrated?: boolean; analyticsSource?: string }) {
  ...
  return <section className={`ep-diagnostic-flow ep-diagnostic-flow--hero ep-diagnostic-flow--step-${step}${integrated ? " ep-diagnostic-flow--integrated" : ""}${routeFirst ? " ep-diagnostic-flow--route-first" : ""}`} aria-label="Diagnóstico inicial da viagem">
    {routeFirst ? <header className="ep-flow-journey-header" aria-label="Resumo atualizado da viagem"><RoutePreview origin={route.origin} destination={route.destination} period={route.period} petCount={selectedPetCount} /></header> : null}
