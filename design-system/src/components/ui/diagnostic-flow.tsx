...
export function DiagnosticFlow({ onComplete, onRouteChange, initialRoute, routeFirst = false, startAtPet = false, integrated = false, analyticsSource = routeFirst ? "mobile_header" : "hero_form" }: { onComplete?: (lead: PublicLead) => void; onRouteChange?: (route: RouteData) => void; initialRoute?: Partial<RouteData>; routeFirst?: boolean; startAtPet?: boolean; integrated?: boolean; analyticsSource?: string }) {
  ...
  return <section className={`ep-diagnostic-flow ep-diagnostic-flow--hero ep-diagnostic-flow--step-${step}${integrated ? " ep-diagnostic-flow--integrated" : ""}${routeFirst ? " ep-diagnostic-flow--route-first" : ""}`} aria-label="Diagnóstico inicial da viagem">
    ...
    {step === 4 && !sent && <div className="ep-flow-card"><p className="ep-flow-kicker">Quase pronto</p><h2 className="ep-flow-title">Para quem enviamos a análise?</h2><div className="ep-flow-summary"><span className="ep-flow-summary__icon"><Plane size={21} aria-hidden="true" /></span><span className="ep-flow-summary__content"><small>Sua rota</small><b>{route.origin} <i>→</i> {route.destination}</b><em>{pets.length} {pets.length === 1 ? "pet" : "pets"} · {route.period}</em></span></div><div className="ep-flow-fields ep-flow-fields--stack"><label className="ep-field"><span>Seu nome</span><span className="ep-contact-input"><UserRound size={16} aria-hidden="true" /><input value={contact.name} onChange={(event) => setContact((current) => ({ ...current, name: event.target.value }))} placeholder="Como podemos chamar você?" /></span></label><div className="ep-field"><span>WhatsApp com DDI</span><span className="ep-contact-input ep-contact-input--phone" onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setPhoneCountryOpen(false); }}><button className="ep-phone-country-trigger" type="button" aria-label={`País do WhatsApp: ${phoneCountry.name}`} aria-haspopup="listbox" aria-expanded={phoneCountryOpen} onClick={() => setPhoneCountryOpen((current) => !current)}><img src={`https://flagcdn.com/w40/${phoneCountry.code.toLowerCase()}.png`} alt={`Bandeira de ${phoneCountry.name}`} width="24" height="18" /><small>{phoneCountry.dial}</small><ChevronDown size={13} aria-hidden="true" /></button>{phoneCountryOpen ? <span className="ep-phone-country-options" role="listbox" aria-label="Selecione o país do WhatsApp">{phoneCountries.map((country) => <button type="button" role="option" aria-selected={country.code === phoneCountry.code} className={country.code === phoneCountry.code ? "is-selected" : ""} key={country.code} onClick={() => { setPhoneCountry(country); setPhoneCountryOpen(false); }}><img src={`https://flagcdn.com/w40/${country.code.toLowerCase()}.png`} alt="" width="24" height="18" /><span><b>{country.name}</b><small>{country.dial}</small></span>{country.code === phoneCountry.code ? <Check size={14} aria-hidden="true" /> : null}</button>)}</span> : null}<input aria-label="Número do WhatsApp" inputMode="tel" value={contact.phone} onChange={(event) => setContact((current) => ({ ...current, phone: event.target.value.replace(/\D/g, "") }))} placeholder="Número do WhatsApp" /></span><small className="ep-phone-hint">Preenchido pelo país de origem. Você pode alterar o país.</small></div></div><FlowActions back={() => go(3)} next={complete} nextLabel="Ver meu resumo" disabled={!contact.name || !contact.phone} /></div>}
...
function RoutePreview({ origin, destination, period, petCount }: RouteData & { petCount: number }) {
  ...
  return <div className="ep-flow-route-preview" aria-live="polite"><span className="ep-flow-route-preview__icon"><Plane size={18} aria-hidden="true" /></span><span className="ep-flow-route-preview__content"><small>Sua rota</small><span className="ep-flow-route-preview__main"><b><span title={origin}>{origin || "Origem"}</span><i>→</i><span title={destination}>{destination || "Destino"}</span></b><span className="ep-flow-route-preview__meta"><em>{petLabel}</em><em>{period || "Defina o prazo"}</em></span></span></span></div>;
}
...
function FlowActions({ back, next, nextLabel, disabled }: { back?: () => void; next: () => void; nextLabel: string; disabled?: boolean }) {
  ...
  const advance = () => { if (disabled) { setShowValidation(true); return; } setShowValidation(false); next(); };
  return <div className={`ep-flow-actions ${back ? "ep-flow-actions--split" : "ep-flow-actions--single"}`}>{showValidation ? <small className="ep-flow-validation" role="alert">Preencha as informações desta etapa para continuar.</small> : null}{back ? <button className="ep-button ep-button--text" type="button" onClick={back}><ArrowLeft size={16} />Voltar</button> : null}<button className="ep-button ep-button--primary" type="button" aria-disabled={disabled} onClick={advance}>{nextLabel}<ArrowRight size={16} /></button></div>;
}
